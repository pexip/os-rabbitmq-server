...
-define(LAST_LINE, "q(quit) s(sort by size) m(sort by memory) pd/pu(page:down/up) F/B(forward/back)" ++
...
manager(ChildPid, #view_opts{db = DBOpts
    = #db{cur_page = CurPage, hide_sys = Hide}} = HomeOpts) ->
...
render_mnesia(MnesiaList, Attr, Rows, CurPage) ->
    ...
    Title = ?render([
        ?UNDERLINE, ?W2(?GRAY_BG, "Name", 25),
        ?UNDERLINE, ?W2(MemColor, "    Memory    ", 16),
        ?UNDERLINE, ?W2(SizeColor, "Size", 16),
        ?UNDERLINE, ?W2(?GRAY_BG, "Type", 12),
        ?UNDERLINE, ?W2(?GRAY_BG, "Storage", 15),
        ?UNDERLINE, ?W2(?GRAY_BG, "Owner", 14),
        ?UNDERLINE, ?W2(?GRAY_BG, "Index", 11),
        ?UNDERLINE, ?W2(?GRAY_BG, "Reg_name", 20)
        ]),
    View =
        [begin
